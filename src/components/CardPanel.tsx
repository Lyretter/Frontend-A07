        ...
        <div>
                    ...
                    <Link key={venueItem.vid} href={`/venue/${venueItem.vid}`} className="w-1/5">
                        <Card
                            ...
                            onRatingChange={(venue: string, value: number) => {
                                ...
                                setVisibleVenues(prev => new Set(prev).add(venue))
                            }}
                        />
                    </Link>
                ))}
            ...
            <div className="px-10">
                <div className="w-full text-xl font-medium mb-4">
                    ...
                    Venue List with Ratings : {Array.from(venueRatings).filter(([venue]) => visibleVenues.has(venue)).length}
                </div>
                
                {Array.from(venueRatings)
                    
                    .filter(([venue]) => visibleVenues.has(venue))
                    .map(([venue, rating]) => (
                        <div 
                            key={venue} 
                            data-testid={venue}
                            ...
                            onClick={() => removeVisibleVenue(venue)}
                        >
                            {venue} Rating : {rating}
                        </div>
                ))}
